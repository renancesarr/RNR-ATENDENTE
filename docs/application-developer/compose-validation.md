...
- Assim que o healthcheck da Evolution API responder `200`, `start.sh` aciona `scripts/evolution-login.sh` (shell puro utilizando `curl`/`jq`) para solicitar o QR code da instância configurada (`EVOLUTION_INSTANCE_NAME`, padrão `mvp-bot`) e salvar um PNG temporário (exibindo também o base64). O `start.test.sh` mantém os serviços ativos por padrão; utilize `--no-keep-running` se quiser que ele encerre automaticamente após a validação.
